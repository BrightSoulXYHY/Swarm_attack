...
REM This script is for large scale UAV swarm simulation
REM This script will automatically ajustify the vehicle ID to the Pixhawk/Mavlink SysID.
REM You don't have to worry about the COM ports randomly assigned by the Windows to mismatch the Pixhawk wit vehicle ID
REM Be sure to change the Pixhawk's SysID through QGC to index the vehicle sequence to use this script
...
%PSP_PATH%\Python38\python.exe _mav_reboot.py
C:
...
:split1
    ...
    REM a 1 is added to the end of the following line to enable CopterID ajustment with Pixhawk/Mavlink SysID
    start CopterSim.exe 1 %cntr% %portNum% %DLLModel% %SimMode% %UE4_MAP% %IS_BROADCAST% %PosXX% %PosYY% %ORIGIN_YAW% %subStr% %UDPSIMMODE% 1
    choice /t 1 /d y /n >nul
    set /a cntr=%cntr%+1
...
cd %PSP_PATH%\RflySimAPIs\_Swarm_Attack
start %PSP_PATH%\Python38\python.exe _add_obj.py
